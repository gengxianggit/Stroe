...
<head>
 ...
 .box-hd .abc{
  ...
  $("#Provinces").bind("change",function(){
	var pname=$(this).find("option:selected").val();
		...
		$.ajax({
			...
			success:function(data){
			 $("#Citys").html("");
			 $("#Citys").append("<option>"+"城市/地区/自治州"+"</option>");
			$.each(data,function(){
				
			var str="<option>"+this.city+"</option>";
			$("#Citys").append(str);
	...
	var cname=$(this).find("option:selected").val();
		...
		$.ajax({
			...
			success:function(data){
			 $("#Countys").html("");
			  $("#Countys").append("<option>"+"县区"+"</option>");
			$.each(data,function(){
				
			 var str="<option>"+this.area+"</option>";
			$("#Countys").append(str);
	...
	$(".Money").html(price);
	 ...
	 $("#J_editAddrOk").click(function(){
		     ...
		     var pro =$("#Provinces").find("option:selected").html();
	        var city = $("#Citys").find("option:selected").html();
			var con=$("#Countys").find("option:selected").html();
		...
		$("#setorder").click(function(){
			...
			$(".sum").each(function(index,element){
			 ...
			 addressId=$("#checkoutAddrList").find(".selected").data("id");
			if(addressId==null){
				alert("请选中地址");
			}else{
				$(".ids").each(function(index,elemnet){
					ids+=$(this).data("id")+",";
					
					
				})
			ids=ids.substring(0,ids.length-1)
  location.href="addorder.do?orderprice="+allprice+"&ordernum="+allnum+"&addressId="+addressId+"&ids="+ids;
...
<header id="pc-header">
	<div class="pc-header-nav">
		<div class="pc-header-con">
			<div class="fl pc-header-link" >您好！，欢迎来云购物  <c:if test="${ empty userNameSession}"><a href="login.do" target="_blank">请登录</a> <a href="register.do" target="_blank"> 免费注册</a></c:if></div>
			<div id="adc">46547465456454</div>
			
			<div class="fr pc-header-list top-nav">
				<ul>
					<li>
						<div class="nav"><i class="pc-top-icon"></i><a href="setorder.do">我的订单</a></div>
						<div class="con">
							<dl>
								<dt><a href="">批发进货</a></dt>
	...
	<div class="yHeader">
		...
		<div class="yNavIndex">
			<ul class="yMenuIndex" style="margin-left:0">
				<li style="background:#d1201e"><a href="index.do" target="_blank">云购首页</a></li>
						...
						<div class="xm-box">
							...
							<div class="box-bd">
								<div class="clearfix xm-address-list" id="checkoutAddrList">
									...
									<dl class="item address"  data-id="${address.id }">
									
										<dt>
											<strong class="itemConsignee">${address.name}</strong>
											<span class="itemTag tag">${address.tag}</span>
										</dt>
										<dd>
											<p class="tel itemTel" >${address.phone }</p>
											<p class="itemRegion" >${address.pro } ${address.city } ${address.con }</p>
											<p class="itemStreet">${address.street }</p>
											<span class="edit-btn J_editAddr deleteAddress" >删除</span>
										</dd>
									...
									<div class="item use-new-addr"  id="J_useNewAddr" data-state="off">
										<span class="iconfont icon-add"><img src="images/add_cart.png"  id="addAdress"/></span>
										使用新地址
									</div>
								</div>
					...
					<div class="checkout-box-ft">
						...
						<div id="checkoutGoodsList" class="checkout-goods-box">
							<div class="xm-box">
								...
								<c:forEach items="${dd.scdList}" var="cd">
										<dd class="item clearfix">
											<div class="item-row">
												<div class="col col-1">
													<div class="g-pic ids" data-id="${cd.comm.id }">
														<img src="img/${cd.comm.pictures.get(0).path}" width="40" height="40" />
													</div>
													<div class="g-info">
														<a href=""detail.do?id=${cd.comm.id}" " target="_blank">
															${cd.comm.name} 
														</a>
													</div>
												</div>
												<div class="col col-2">${cd.comm.price}</div>
												<div class="col col-3 sum" >${cd.num }</div>
												<div class="col col-4 subtotal" >${ cd.subtotal }</div>
											</div>
										</dd>
										</c:forEach>
									...
									<div class="checkout-count clearfix">
										...
										<div class="checkout-price">
											...
											<p class="checkout-total">应付总额：<span><strong id="totalPrice">${price }</strong>元</span></p>
										</div>
										<!--  -->
									</div>
								</div>
						...
						<div class="checkout-confirm">
							...
							<a href="cart.do" class="btn btn-lineDakeLight btn-back-cart">返回购物车</a>
							<a  class="btn btn-primary" id="setorder">立即下单</a>
