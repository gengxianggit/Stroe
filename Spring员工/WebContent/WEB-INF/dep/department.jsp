...
<div>
   <form class="form-inline" role="form" action="search.do">
    ...
    <input type="text" class="form-control" name="num" value='${c.num !=-1?c.num:""}'placeholder="请输入个数">
...
<br/>
		...
		<ul class="pagination">
	<li  id="pre"><a href="search.do?ye=${p.ye-1}&name=${c.name }&num=${c.num !=-1?c.num:""}">上一页</a></li>
     <c:forEach begin="${p.beginYe}" end="${p.endYe}" varStatus="status">
	<li <c:if test="${p.ye==status.index}"> class="active" </c:if> >
	<a href="search.do?ye=${status.index}&name=${c.name }&num=${c.num !=-1?c.num:""}"> ${status.index}</a></li>
	</c:forEach>
	<li  <c:if test="${p.ye==status.index}"> class="active" </c:if> id="next"><a href="search.do?ye=${p.ye+1}&name=${c.name }&num=${c.num !=-1?c.num:""}">下一页</a></li>
</ul>
