...
$().ready(function(){
	$(".orderDetail").click(function(){
		var id=$(this).data("id");
		location.href="orderdetailed.do?id="+id;
	...
	var num=$(".searchName").val();
	var time=$(".searchTime").val();
	location.href="searchNN.do?num="+num+"&time="+time;
  ...
  <div class="h_products_list clearfix" id="products_list">
     ...
     <div class="table_menu_list order_style" id="table_order_list">
      ...
      <ul class="search_content clearfix">
       <li><label class="l_f">订单编号</label><input name="" type="text" class="text_add searchName" placeholder="订单订单编号" style=" width:250px"></li>
       <li><label class="l_f">时间</label><input class="inline laydate-icon searchTime" id="start" style=" margin-left:10px;"></li>
       <li style="width:90px;"><button type="button" class="btn_search search"><i class="fa fa-search"></i>查询</button></li>
      </ul>
    </div>
     <!--订单列表展示-->
    ...
    <c:forEach items="${order.odlist }" var="odd"  varStatus="ab">
      <a ><img src="pic/${odd.pro.pictures.get(0).path }"  title="产品名称" alt="产品下架"/></a>
     <c:if test="${! ab.last }"><i class="fa fa-plus"></i></c:if>
     ...
     <td>${order.orderNum }</td>
      <td class="td-status"><span class="label label-success radius">${order.goodCondition }</span></td>
     <td>
     <a onClick="Delivery_stop(this,'10001')"  href="javascript:;" title="发货"  class="btn btn-xs btn-success"><i class="fa fa-cubes bigger-120 fahuo" data-id="${order.id }"></i></a> 
     <a title="订单详细"   class="btn btn-xs btn-info order_detailed" ><i class="fa fa-list orderDetail bigger-120" data-id="${order.id }"></i></a> 
