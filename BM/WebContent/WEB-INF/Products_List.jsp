...
$().ready(function(){
	$("#deletebatch").click(function(){
		var ids="";
		$(".mutilace").each(function(element,index){
		 if($(this).prop("checked")==true){
			 ids+=$(this).parent().parent().parent().find("#pid").html()+",";
	...
	ids=ids.substring(0,ids.length-1);
		location.href="deleteBatch.do?ids="+ids;
	})
 ...
 <div id="products_style">
    <div class="search_style">
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="showAdd.do" title="添加商品" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加商品</a>
        <a href="javascript:ovid()" class="btn btn-danger" id="deletebatch"><i class="icon-trash"></i>批量删除</a>
       </span>
       <span class="r_f">共：<b>${pList.size()}</b>件商品</span>
     </div>
     <!--产品列表展示-->
	...
	<c:forEach items="${pList}" var="p">
     <tr>
        <td width="25px"><label><input type="checkbox" class="ace mutilace" ><span class="lbl"></span></label></td>
        <td width="80px" id="pid">${p.id }</td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick="">${p.name } ${p.intriduceproduction }</u></td>
        <td width="100px">${p.price }</td>
        <td width="100px">${p.price-10 }</td> 
        <td width="100px">中国</td>         
        <td width="180px">2014-6-11 11:11:42</td>
        <td class="text-l">通过</td>
        <td class="td-status"><span class="label label-success radius">已启用</span></td>
        <td class="td-manage">
        <a onClick="member_stop(this,'10001')"  href="javascript:;" title="停用"  class="btn btn-xs btn-success" data-id="${p.id }"><i class="icon-ok bigger-120"></i></a> 
        <a title="编辑"  class="btn btn-xs btn-info updateProduction" ><i class="icon-edit bigger-120"  ></i></a> 
        <a title="删除" href="javascript:;"  onclick="member_del(this,'1')" class="btn btn-xs btn-warning" data-id="${p.id }"><i class="icon-trash  bigger-120"></i></a>
       </td>
	  </tr>
	  </c:forEach>
<!--        <tr> -->
	...
	callback: {
		beforeClick: function(treeId, treeNode) {
			...
			if (treeNode.isParent) {
				...
				demoIframe.attr("src",treeNode.file + ".do");
				return true;
			}
		}
	}
...
function member_del(obj,id){
	var id=$(obj).data("id");
	location.href="deleteProduction.do?id="+id;
